...
class BinningTest(absltest.TestCase):
  ...
  def testBucketBoundaries(self):
    ...
    result, bin_counts = bin_and_aggregate(
  ...
  def testMeanAggr(self):
    ...
    y = np.array([0, -1, 1, 4, 5, 6, 1, 3, 3, 9, 1, 1, 1, 1, -2])
    result, bin_counts = bin_and_aggregate(
        x, y, num_bins=5, bin_width=2, x_min=-5, x_max=5, aggr_fn=np.mean)
  ...
  def testMinAggr(self):
    ...
    y = np.array([0, -1, 1, 4, 5, 6, 1, 3, 3, 9, 1, 1, 1, 1, -2])
    result, bin_counts = bin_and_aggregate(
        x, y, num_bins=5, bin_width=2, x_min=-5, x_max=5, aggr_fn=np.min)
  ...
  def testMaxAggr(self):
    ...
    y = np.array([0, -1, 1, 4, 5, 6, 1, 3, 3, 9, 1, 1, 1, 1, -2])
    result, bin_counts = bin_and_aggregate(
        x, y, num_bins=5, bin_width=2, x_min=-5, x_max=5, aggr_fn=np.max)
  ...
  def test2D(self):
    ...
    result, bin_counts = bin_and_aggregate(
        x, y, num_bins=5, bin_width=2, x_min=-5, x_max=5)
    np.testing.assert_array_equal([[0, 123], [0, 1], [5, 4], [3, 100], [1, 0]],
                                  result)
    np.testing.assert_array_equal([1, 2, 3, 4, 5], bin_counts)
  ...
  def test2DMean(self):
    ...
    np.testing.assert_array_almost_equal(
        [[0, 123], [0, 1], [5, 2], [4, 90], [0.4, 1]], result)
    np.testing.assert_array_equal([1, 2, 3, 4, 5], bin_counts)
